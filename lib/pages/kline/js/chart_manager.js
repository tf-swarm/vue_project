...
export class ChartManager {
  ...
  setCaptureMouseWheelDirectly(v) {
    ...
    if (v) $(this._overlayCanvas).bind('mousewheel', Control.mouseWheel);else $(this._overlayCanvas).unbind('mousewheel');
  }
  ...
  drawMain(frameName, context) {
    ...
    if (!drawn) {
      for (let it in this._areas) {
        if (this._areas[it].getFrameName() === frameName && !Util.isInstance(this._areas[it], areas.ChartAreaGroup)) this.drawAreaMain(context, this._areas[it]);
      }
    }
    let e;
  ...
  drawOverlay(frameName, context) {
    for (let n in this._areas) {
      ...
      if (Util.isInstance(area, areas.ChartAreaGroup)) if (area.getFrameName() === frameName) {
        area.drawGrid(context);
      }
    }
    for (let n in this._areas) {
      let area = this._areas[n];
      if (Util.isInstance(area, areas.ChartAreaGroup) === false) if (area.getFrameName() === frameName) {
  ...
  onToolMouseMove(frameName, x, y) {
    ...
    for (let n in this._dataSources) {
      if (n.indexOf(frameName) === 0) {
        ...
        if (Util.isInstance(ds, data_sources.MainDataSource)) if (ds.toolManager.acceptMouseMoveEvent(x, y)) ret = true;
      }
    }
    return ret;
  }
  ...
  onToolMouseDown(frameName, x, y) {
    ...
    for (let n in this._dataSources) {
      if (n.indexOf(frameName) === 0) {
        let ds = this._dataSources[n];
        if (Util.isInstance(ds, data_sources.MainDataSource)) if (ds.toolManager.acceptMouseDownEvent(x, y)) ret = true;
      }
    }
    return ret;
  }
  ...
  onToolMouseUp(frameName, x, y) {
    ...
    for (let n in this._dataSources) {
      if (n.indexOf(frameName) === 0) {
        let ds = this._dataSources[n];
        if (Util.isInstance(ds, data_sources.MainDataSource)) if (ds.toolManager.acceptMouseUpEvent(x, y)) ret = true;
      }
    }
    return ret;
  }
  ...
  onToolMouseDrag(frameName, x, y) {
    ...
    for (let n in this._dataSources) {
      if (n.indexOf(frameName) === 0) {
        let ds = this._dataSources[n];
        if (Util.isInstance(ds, data_sources.MainDataSource)) if (ds.toolManager.acceptMouseDownMoveEvent(x, y)) ret = true;
      }
    }
    return ret;
  }
    ...
    if (this._capturingMouseArea) {
      if (this._capturingMouseArea.onMouseUp(x, y) === null && this._dragStarted === false) {
        ...
        if (this._capturingMouseArea.isSelected()) {
          ...
          if (this._selectedFrame !== frame) if (!this._captureMouseWheelDirectly) $(this._overlayCanvas).bind('mousewheel', Control.mouseWheel);
  ...
  setMainIndicator(dsName, indicName) {
    ...
    if (dp === undefined || !Util.isInstance(dp, data_providers.MainDataProvider)) return false;
    let indic;
    switch (indicName) {
      case 'MA':
        indic = new indicators.MAIndicator();
  ...
  removeMainIndicator(dsName) {
    ...
    let indicDpName = areaName + '.secondary';
    let indicDp = this.getDataProvider(indicDpName);
    if (indicDp === undefined || !Util.isInstance(indicDp, data_providers.IndicatorDataProvider)) return;
  ...
  removeIndicator(areaName) {
    ...
    let dp = this.getDataProvider(areaName + '.secondary');
    if (dp === undefined || !Util.isInstance(dp, data_providers.IndicatorDataProvider)) return;
  ...
  setIndicatorParameters(indicName, params) {
    ...
    for (n in this._dataProviders) {
      ...
      if (Util.isInstance(dp, data_providers.IndicatorDataProvider) === false) continue;
