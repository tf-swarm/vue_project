  ...
  <section>
    ...
    <el-dialog :title="form && form.id ? '编辑' : '新增'" :visible.sync="formVisible" :close-on-click-modal="false">
      <el-form :model="form" label-width="120px" :rules="rules" ref="form" label-position="left">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="报价缓冲" prop="quote_buffer" :rules="[{ required: true, message: ' ', trigger: 'blur' }]">
              <el-input type="text" v-model="form.quote_buffer" placeholder="" clearable suffix-icon="el-icon-edit" :disabled="viewDisabled" v-positive="'float'" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="日波动率" prop="day_rate" :rules="[{ required: true, message: ' ', trigger: 'blur' }]">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item  label="报单间隔(秒)" prop="min_waitTime" :rules="[{ required: true, message: ' ', trigger: 'blur' }]">
              <el-input v-model="form.min_waitTime" style="width: 110px;"></el-input>
              <span>--</span>
              <el-input v-model="form.max_waitTime" style="width: 110px;"></el-input>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item  label="报单数量限值" prop="amount_start" :rules="[{ required: true, message: ' ', trigger: 'blur' }]">
              <el-input v-model="form.amount_start" style="width: 110px;"></el-input>
              <span>--</span>
              <el-input v-model="form.amount_end" style="width: 110px;"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
		...
		<el-row :gutter="20">
		  <el-col :span="12">
		    <el-form-item label="报单基准数量" prop="base_amount" :rules="[{ required: true, message: ' ', trigger: 'blur' }]">
		      <el-input type="text" v-model="form.base_amount" placeholder="" clearable suffix-icon="el-icon-edit" :disabled="viewDisabled" v-positive="'float'" />
		    </el-form-item>
		  </el-col>
...
let data = function () {
  return {
    ...
    tableList: [
	    { label: '策略ID', prop: 'bucketing_id', min: '180' },
...
let getRows = function () {
  ...
  this.$post('/brushOrder/findPage', param)
    .then(res => {
...
let submit = function () {
  ...
  this.$refs.form.validate(valid => {
    ...
    this.$post('/brushOrder/save', this.form)
      .then(res => {
...
let remove = function (id) {
  ...
  this.$confirm('确定要删除吗?', '提示', {
    ...
    .then(() => {
      let params = { id }
      this.listLoading = true
      this.$get('/brushOrder/remove', params)
        .then(res => {
          this.listLoading = false
          if (res.status != 0) {
            this.$message({
...
let startRun = function (id) {
  ...
  this.listLoading = true
  let params = { id }
  this.$get('/brushOrder/start', params)
    .then(res => {
      this.listLoading = false
      if (res.status != 0) {
        this.$message({
...
let stopRun = function (id) {
  ...
  this.listLoading = true
  let params = { id }
  this.$get('/brushOrder/stop', params)
    .then(res => {
      this.listLoading = false
      if (res.status != 0) {
        this.$message({
